    ...
    <div class="overflow">
      <div class="overflow">
          <table class="layui-table" cellspacing="0" cellpadding="0" border="0">
            ...
            <tbody v-for="(item, index) in list " :key="'mypost' + index ">
              <tr class="text-centre">
                <td>
                  <router-link class="link" :to="{
                    name: 'detail',
                    params: { tid: item._id}}">{{item.title}}</router-link>
                </td>
                <td>{{item.status === '0' ? '打开' : '关闭'}}</td>
                <td :class="{'success': item.isEnd !== '0'}">{{item.isEnd === '0' ? '未结' : '已结'}}</td>
                <td>{{item.created | moment}}</td>
                <td>{{item.reads}}阅/{{item.answer}}答</td>
                <td>
                  <div :class="{'layui-btn-disabled': item.isEnd === '1'}" class="layui-btn layui-btn-xs" @click="editPost(item)">编辑</div>
