<template>
  ...
  <img :src="userInfo.pic ? userInfo.pic : '/img/Neolu.png'">
  ...
  <div class="layui-row layui-col-space15">
    <div class="layui-col-md6 fly-home-jie">
      <div class="fly-panel">
        ...
        <ul class="jie-row">
          <li v-for="(item, index) in postList" :key="'mypost' + index">
            <span
              class="layui-badge layui-bg-red"
              v-for="(tag,index) in item.tags"
              :key="'tag'+index"
              :class="tag.class">{{tag.name}}
            </span>
            <router-link class="jie-title" :to="{name: 'detail', params: { tid: item._id} }"> {{item.title}}</router-link>
    ...
    <div class="layui-col-md6 fly-home-da">
      <div class="fly-panel">
        ...
        <ul class="home-jieda">
          ...
          在<router-link :to="item.tid ? {name: 'detail', params: {tid: item.tid ? item.tid._id : '' }} : {name: '404'} ">{{item.tid ? item.tid.title : '(用户已经删除该帖)'}}</router-link>中回答：
          </p>
...
<script>
import { getInfo, getCommentList, getPostListByUid } from '@/api/user'
export default {
  name: 'home',
  props: ['uid'],
  data () {
